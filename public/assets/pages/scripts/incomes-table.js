var TableDatatablesEditable = function () {
    ...
    var handleTable = function () {
        ...
        function loadAjax()
        {
             $.get('/incomes/create', function (data) {
                var types = data.types;
                var methods = data.methods;
                 $('#income_type').empty();
                            $.each(types, function (index, type) {
                            $('#income_type').append($("<option></option>").attr('value', type.id).text(type.name));
                            });
        ...
        function editAjax(id)
        {
             $.get('/incomes/' + id + '/edit', function (data) {
                ...
                            $.each(methods, function (index, type) {
                            $('#payment_method').append($("<option></option>").attr('value', type.id).text(type.name));
                            });
                $('#income_type').val(income.income_type_id);
                $('#payment_method').val(income.method_id);
                         
                    });
        }
        function editRow(oTable, nRow) {
            ...
            jqTds[1].innerHTML = '<select id="income_type" name="income_type" class="form-control input-small"></select>';
            jqTds[2].innerHTML = '<input type="text" class="form-control input-small is_required" value="' + aData[2] + '">';
        ...
        function saveRow(oTable, nRow) {
            ...
            var incomeTypes = $('#income_type :selected').text();
            var keys = {};
            ...
            oTable.fnUpdate(incomeTypes, nRow, 1, false);
